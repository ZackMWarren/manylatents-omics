...
class TestMalformedDatasetRejected:
    ...
    def test_empty_matrix_is_rejected(self, monkeypatch):
        ...
        with pytest.raises(ValueError, match="no var_names"):
            self._load(monkeypatch, self._adata(n_genes=0))

    def test_missing_time_labels_is_rejected(self, monkeypatch):
        # use_time requested, but barcodes carry no '-<int>' time suffix.
        bad = self._adata(barcodes=["CellA", "CellB", "CellC", "CellD"])
        with pytest.raises(ValueError, match="time"):
            self._load(monkeypatch, bad, use_time=True)
    ...
    def test_from_anndata_rejects_empty_directly(self):
        ...
        adata = self._adata(n_cells=3, n_genes=0)
...
def _selected_datasets(n: int = 5):
    """Pick ``n`` datasets from the manifest registry; ``[]`` if it is absent.
...
@pytest.mark.network
@pytest.mark.slow
@pytest.mark.skipif(not _SELECTED_DATASETS, reason="dataset manifest not found")
@pytest.mark.parametrize(
    "entry",
    _SELECTED_DATASETS,
    ids=[_short_id(e.name) for e in _SELECTED_DATASETS],
)
